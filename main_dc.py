...
from utils_.sparsity_check import Sparsity_check
...
################# Options ##################################################
############################################################################
parser = argparse.ArgumentParser(description='Trains Network on CIFAR or ImageNet',
                                 formatter_class=argparse.ArgumentDefaultsHelpFormatter)
parser.add_argument('--data_path', default='/home/elliot/data/pytorch/svhn/', type=str, help='Path to dataset')
...
parser.add_argument('--schedule', type=int, nargs='+', default=[80, 110],
                    help='Decrease learning rate at these epochs.')
parser.add_argument('--gammas', type=float, nargs='+', default=[0.5, 0.2],
...
parser.add_argument('--model_only', default=False, type=bool, help='only save the model without external utils_')
# Acceleration
parser.add_argument('--ngpu', type=int, default=1, help='0 = CPU.')
parser.add_argument('--gpu_id', type=int, nargs='+', default=0, help='device range [0,ngpu-1]')
...
def main():
    ...
    if args.optimizer == "SGD":
        ...
        optimizer_th = torch.optim.SGD(th_param,
                                       lr=state['learning_rate'],
                                       momentum=state['momentum'], weight_decay=0, nesterov=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        current_learning_rate, current_momentum = adjust_learning_rate(optimizer_th, epoch, args.gammas, args.schedule)
        ...
        # train for one epoch
        train_acc, train_los = train(train_loader, net, criterion, optimizer, optimizer_th, epoch, log)
        ...
        for name, param in net.named_parameters():
            name = name.replace('.', '/')
            writer.add_histogram(name + '/grad', param.grad.cpu().detach().numpy(), epoch + 1)
        ...
        for name, module in net.named_modules():
            ...
            if "quanConv2d" in class_name or "quanLinear" in class_name:
                if module.delta_th.data is not None:
                    if module.delta_th.dim() == 0:  # zero-dimension tensor (scalar) not iterable
                        writer.add_scalar(name + '/delta/', module.delta_th.detach(), epoch + 1)
                    else:
                        for idx, delta in enumerate(module.delta_th.detach()):
                            writer.add_scalar(name + '/delta/' + '{}'.format(idx), delta, epoch + 1)
...
def train(train_loader, model, criterion, optimizer, optimizer_th, epoch, log):
