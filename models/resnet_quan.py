...
def quan_resnet18b_ff_lf(num_classes=1000):
    ...
    state_tmp.update(model_zoo.load_url(model_urls['resnet18']))
    model.load_state_dict(state_tmp)
    return model

def quan_resnet18b_fq_lq(num_classes=1000):
...
def quan_resnet34b_fq_lq(num_classes=1000):
    model = ResNet(BasicBlock, [3, 4, 6, 3], fp_fl=False, fp_ll=False)
    state_tmp = model.state_dict()
    state_tmp.update(model_zoo.load_url(model_urls['resnet34']))
    model.load_state_dict(state_tmp)
    return model


def quan_resnet50b_fq_lq(num_classes=1000):
    model = ResNet(Bottleneck, [3, 4, 6, 3], fp_fl=False, fp_ll=False)
    state_tmp = model.state_dict()
    state_tmp.update(model_zoo.load_url(model_urls['resnet50']))
    model.load_state_dict(state_tmp)
    return model